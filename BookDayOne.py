...
def recommendation_page():
    ...
    try:
            ...
            st.text(f"Total amount invested over {years_to_invest} years (adjusted for inflation): ${total_invested:,.2f}")
            st.text(f"The median projected portfolio value at the end of the investment period (considering inflation) is: ${median_projection[-1]:,.2f}")
            st.text(f"The projected portfolio value range is from ${lower_bound[-1]:,.2f} to ${upper_bound[-1]:,.2f} (5th to 95th percentile)")
